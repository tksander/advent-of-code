...
{
    int sum;
    int matrix[16][16] = {{278,	1689,	250,	1512,	1792,	1974,	175,	1639,	235,	1635,	1690,	1947,	810,	224,	928,	859},
    ...
    size_t numElements = sizeof(matrix) / sizeof(matrix[0]);
    int getRowSum(int row[], size_t numElements);
    int getCheckSum(int *matrix[16], size_t numElements);

    sum = getCheckSum(matrix, numElements);
    printf("Checksum: %d\n", sum);
    return 0;
}
...
int getCheckSum(int *matrix[16], size_t numElements)
{
    ...
    for (int i = 0; i < numElements; i++) {
        size_t rowSize = sizeof(matrix[i]) / sizeof(int);
        sum = sum + getRowSum(matrix[i], rowSize);
    }
    return 0;
}
